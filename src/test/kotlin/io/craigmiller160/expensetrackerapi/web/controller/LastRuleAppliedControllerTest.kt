...
constructor(
  ...
  private fun createRule(): Pair<AutoCategorizeRule, LastRuleApplied> {
    ...
    val lastApplied =
      lastRuleAppliedRepository.saveAndFlush(
        LastRuleApplied(userId = 1L, transactionId = transaction.recordId, ruleId = rule.recordId))
  ...
  fun `getLastAppliedRuleForTransaction - last applied exists`() {
    ...
    mockMvc
      .get("/transactions/rules/last-applied/${transaction.recordId}") {
        secure = true
